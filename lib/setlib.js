// step
const installStep = document.getElementById('install');
installStep.onclick = function(){

    window.location = "../index.html";
}
const ImgStep = document.getElementById('img');
ImgStep.onclick = function(){
    window.location = "../pages/img.html";
}
const ButtonStep = document.getElementById('button');
ButtonStep.onclick = function(){
    window.location = "../pages/buttons.html";
}
const alertStep = document.getElementById('alert');
alertStep.onclick = function(){
    window.location = "../pages/alert.html";
}
const textStep = document.getElementById('text');
textStep.onclick = function(){
    window.location = "../pages/text.html";
}
const columnsStep = document.getElementById('columns');
columnsStep.onclick = function(){
    window.location = "../pages/column.html";
}
const codeStep = document.getElementById('code');
codeStep.onclick = function(){
    window.location = "../pages/code.html";
}
const settingStep = document.getElementById('setting');
settingStep.onclick = function(){
    window.location = "../pages/setting.html";
}
const svgStep = document.getElementById('svg');
svgStep.onclick = function(){
    window.location = "../pages/svg.html";
}
const accordionStep = document.getElementById('accordion');
accordionStep.onclick = function(){
    window.location = "../pages/accordion.html";
}
const stickyStep = document.getElementById('sticky');
stickyStep.onclick = function(){
    window.location = "../pages/sticky.html";
}

const mcStep = document.getElementById('magic-css');
mcStep.onclick = function(){
    window.location = "./magic-css.html";
}
const fabricStep = document.getElementById('fabric');
fabricStep.onclick = function(){
    window.location = "./fabric.html";
}